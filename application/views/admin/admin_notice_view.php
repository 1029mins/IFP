...
?>
	...
	<div class="card shadow mb-4">
	  ...
	  <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between" style="background-color:#4e73df;">
	    ...
	    <div class="dropdown no-arrow">
	      ...
	      <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
	        <a class="dropdown-item" href="/admin_notice/del/no/<?=$no;?><?=$tmp;?>" onClick="return confirm('삭제할까요 ?');" style="color:red;">Delete</a>
	      </div>
	    </div>
	  </div>
	  <!-- Card Body -->
	  <div class="card-body row">
	      ...
	      <div class="card mb-4 py-0">
			...
			<!--contents-->
	            ...
	            <div class="form-inline">
	                ...
	                <div class="col-lg-9">
						<br>
	                    <input type="datetime-local" name="duedate" value="<?=$row->duedate ;?>" class="form-control form-control-sm fill-form-Box">
				...
				<div class="form-inline">
					...
					<div class="col-lg-9" >
						<input type="file" name="filename" value="<?=$row->filename ;?>" class="form-control form-control-sm" >
					</div>
				</div>
                <hr>
                <div class="form-inline">
		...
		<?
            if ($row->filename) // 파일이 있는 경우
            {
                echo("<center><b>파일명</b> : $row->filename <br><br></center>");
            }
            else {          // 파일이 없는 경우
                echo("<center><b>파일없음</b></center><br><br>");
            }
		    if ($row->pic) // 이미지가 있는 경우
			{
				echo("<center><b>사진명</b> : $row->pic <br><br></center>");
		        echo("<center><img src='/images/notice/$row->pic' width='200' ></center>");
			}
			else {          // 이미지가 없는 경우
	...
	<div align="center">
		<!--<a href="/~sale/notice/edit" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm">수정하기</a>&nbsp;-->
		<a href="/admin_notice/lists<?=$tmp;?>" class="d-sm-inline-block btn btn-sm btn-primary shadow-sm">목록</a>&nbsp;
