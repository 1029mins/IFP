<?
    class Admin_docs extends CI_Controller {
        ...
        public function lists()
        {
            ...
            /*pagination*/
			if ($text1=="")
				$base_url = "/admin_docs/lists/page";
			else
				$base_url = "/admin_docs/lists/text1/$text1/page";
			$page_segment = substr_count( substr($base_url,0,strpos($base_url,"page")) , "/" )+1;
			//$base_url = "/~sale26" . $base_url;
			$config["per_page"]	 = 10;								// 페이지당 표시할 line 수
			$config["total_rows"] = $this->admin_docs_m->rowcount($text1);		// 전체 레코드개수 구하기
            ...
            $data["list"] = $this->admin_docs_m->getlist($text1, $start, $limit);
            $this->load->view("admin/admin_header");
            $this->load->view("admin/admin_docs_list",$data);
        ...
        public function view()
        {
           ...
           {
               $data["row"] = $this->admin_docs_m->getrow($no);
               $this->load->view("admin/admin_header");
               $this->load->view("admin/admin_docs_view",$data);
               $this->load->view("admin/admin_footer");
           }
           else
           {                                            //수정버튼 클릭 edit
               ...
               $this->admin_docs_m->updaterow($data, $no);
               $tmp = $text1 ? "/text1/$text1/page/$page" : "/page/$page";
               redirect("/admin_docs/view/no/" . $no . $tmp);
           }
       ...
       public function add()
        {
            ...
            {
                $this->load->view("admin/admin_header");
                $this->load->view("admin/admin_docs_add");
                $this->load->view("admin/admin_footer");
            }
            else
            {   //미완성 - 작성자랑 조회수
                ...
                $this->admin_docs_m->insertrow($data);
                redirect("/admin_docs/lists". $text1 . $page);
            }
        ...
        public function del()
         {
             ...
             $this->admin_docs_m->deleterow($no);
             redirect("/admin_docs/lists" . $text1 . $page);
         }
